...
The configuration file SHOULD include minimal metadata about the paper, i.e. the authors of the paper whose workflow is submitted to the CODECHECK, within a root-level sequence `paper`.
"Authors" is used widely here and includes all contributors, e.g., software engineers, etc.
This information might be added and updated later, e.g., after publication.

The element `paper` SHOULD have a child item `title` with the title of the submission or publication.

The element `paper` SHOULD have a child item `authors`.
The `authors` sequence MUST have at least one child node `name` with the author's name.
