...
redirect_from:
  ...
  # Certificates from 2020 carry it in their version node, and CODECHECK
  # tooling reports an unresolvable version URL - see CC-CFG-015.
  - spec/1.0/
  - spec/1/
---

The checks that CODECHECK tooling applies to this version of the specification are published as a machine-readable list: [`rules-1.0.yml`](https://raw.githubusercontent.com/codecheckers/register/master/rules-1.0.yml) in the [register repository](https://github.com/codecheckers/register/), with the identifier scheme documented in [`RULES.md`](https://github.com/codecheckers/register/blob/master/RULES.md).
...
A direct link to a PDF (e.g. a report hosted on a project website rather than through a publisher or repository) SHOULD be avoided when a DOI or a stable landing page exists instead: such links tend to break as sites are restructured, and unlike a landing page they carry no machine-readable publication metadata (authors, date, venue) for tools like the [CODECHECK register](/register) to read. If a PDF link is genuinely the only reference available (e.g. an informal report with no DOI), prefer archiving it with the [Wayback Machine](https://web.archive.org/) and using the resulting `web.archive.org/web/<timestamp>/<url>` snapshot URL - it is far less likely to 404 later than the original.
...
- `certificate`: A unique identifier for the certificate as awarded in the [CODECHECK register](https://github.com/codecheckers/register/).
